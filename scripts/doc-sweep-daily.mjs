// Daily launcher for the cintamani.doc-sweep SOP occurrence.
// Invoked by the Windows scheduled task \Narada\CintamaniDocSweepDaily.
// Idempotent per day: the occurrence key is doc-sweep:<YYYYMMDD> (UTC).
...
// Usage: node scripts/doc-sweep-daily.mjs [--dry-run]
...
const SOP_ID = 'cintamani.doc-sweep';
const SCHEDULED_TASK = '\\Narada\\CintamaniDocSweepDaily';

const dryRun = process.argv.includes('--dry-run');
const occurrenceKey = `doc-sweep:${new Date().toISOString().slice(0, 10).replace(/-/g, '')}`;
...
const loader = await McpProcessClient.start({
  ...
  env,
  clientName: 'cintamani-doc-sweep-launcher',
});
